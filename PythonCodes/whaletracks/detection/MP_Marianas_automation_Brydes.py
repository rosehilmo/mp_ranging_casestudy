...
sys.path.append('/Users/wader/Documents/GitHub/mp_ranging_casestudy/PythonCodes/whaletracks')
...
IS_RESTART = False #True if you are adding to an existing file, false if you want new files
...
def main(STARTTIME, ENDTIME,
        ...
        is_restart=False, dt_up = dt_up, dt_down = dt_down):
    """
    ...
    while utcend > utcstart_chunk: #loop through chunk times of data so long as the start chunk is less than the end time
        ...
        try: 
            ...
            #Sometimes if the internet hiccups it will skip a lot of dates
            # if the request fails, waits to reset connection then tries again so data isn't skipped
        ...
        for idx in range(1, num_sta+1): #iterates for each station (should only be 1 for this code)
            ...
            for mp_iter in range(numchunks): #iterate for each minute in day, calculates a multipath range for each minute that there are calls
                ...
                #import pdb; pdb.set_trace()
                if len(min_df_sub) >= 1 and len(j_df_sub) >= 3:  #If there is at least one call in the center minute, calculate multipath range for the minute using autocorrelation of detection score
                    ...
                    d2 = {'date': [start_utc+CHUNK_LENGTH/2], 'epoch': datetimeToEpoch([start_utc+CHUNK_LENGTH/2]), 'n_calls': [len(mp_df_auto)], 'sum_calls': [len(j_df_sub)], 'peaks': [
                        np.median(j_df_sub['peak_signal'])], 'snr': [np.median(j_df_sub['snr'])], 'db_amps': [np.median(j_df_sub['db_amps'])], 'low_snr': [np.mean(j_df_sub['eq_snr'])]}
