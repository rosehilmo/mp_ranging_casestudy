...
@author: wader
"""
#This code makes spectrograms and plots bryde's detection times and scores 
#from the files produced by main_detection for verification 

import sys
import os
sys.path.append('/Users/ryan.homer/Documents/GitHub/mulipath_ranging') 
sys.path.append('/Users/ryan.homer/Documents/GitHub/mulipath_ranging/brydes_marianas') 
from obspy.clients.fdsn import Client
from obspy import UTCDateTime
from obspy import read, read_inventory
import pandas as pd
from brydes_marianas.detect_calls import plotwav
import numpy as np
...
CHUNK_FILE='brydes_marianas/DELETEME.csv' #File name where verified calls will be saved
DETECTION_FILE='brydes_marianas/LongCall_YearPicks/B02_mp_Brydes_Year_LongCall.csv' #File name produced by main_detection
is_restart='True' #each iteration reads CHUNK_FILE and adds new verified calls
station_id_list=['B19'] #List each station you want to verify

STARTTIME = ("2012-3-11T7:00:00.000") #start time
print(STARTTIME)
...
for station_ids in station_id_list:   
    ...
    #Filter  dataframes for specific station and date range
    ...
    [f,t,Sxx]=plotwav(tr_filt.stats.sampling_rate, tr_filt.data, filt_freqlim=[12, 45],window_size=1, overlap=.95, plotflag=False)
    t=t+STARTEPOCH 

    #Subsample spectrogram to call range and convert to dB
    freq_inds=np.where(np.logical_and(f>=12, f<=45))
    f_sub=f[freq_inds]
    ...
    #plot spectrogram on lower axis
        ...
        for b_ind in range(0,len(b_start)):
            #ax0.plot([b_start[b_ind],b_end[b_ind]],[b_score[b_ind],b_score[b_ind]],'r') 
    ...
    for inds in range(0,len(true_calls)):
        ...
        if diff < 10:
            ...
            chunk_verified = pd.concat([chunk_verified,b_df_sub.iloc[k].to_frame().T],axis=0,ignore_index=True)
