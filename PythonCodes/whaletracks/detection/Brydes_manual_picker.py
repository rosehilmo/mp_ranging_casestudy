...
sys.path.append('/Users/ryan.homer/Documents/GitHub/whaletracks') #allows us to import my written function 
...
import whaletracks.detection.detectBrydes_manual as detect
from whaletracks.common.util import datestrToEpoch

#Define constants
HALF_HOUR = 1800  # in seconds
CHUNK_LENGTH=HALF_HOUR  #Sets spectrogram time chunk. 15 minutes is ideal for selecting blue whale calls.
PLOTFLAG=True #keep this true since you are manually picking
CHUNK_FILE='DELETEMEBrydes_picks.csv' #csv that is extended with each time period
detection_pth="DELETEMEFinal_brydes_picks.csv" #final .csv name
...
station_id_list=['B19']#Pick 1 station at a time
location='*'  # '*' for all available locations
channel='HHZ,EHZ,ELZ,BHZ,EDH' #Choose channel from specific station,  you'll want 'BHZ,HHZ' for Cascadia
                      #Check http://ds.iris.edu/mda/OO/ for OOI station channels

#Select start and end times for manual picking
STARTTIME = ("2012-7-31T21:30:00.000")
ENDTIME = ("2012-7-31T23:30:00.000")
...
while utcend > utcstart_chunk:
    ...
    print("Start time " + str(utcstart_chunk))
    ...
    for station_ids in station_id_list:  #loops through stations requested. Reccomend 1 station at a time however. 
        ...
        st_raw.remove_response(output='VEL',pre_filt=[8,10,45,47])
        ...
        [f,t,Sxx]=detect.plotwav(tr_filt.stats.sampling_rate, tr_filt.data, window_size=4, overlap=.98, plotflag=False)
        

        #Subsample spectrogram to call range and convert to dB
        freq_inds=np.where(np.logical_and(f>=10, f<=47))
        f_sub=f[freq_inds]
        ...
        #plot spectrogram and prepare for user input
            ...
            print('Select all Brydes calls at starting time and frequency')
        ...
        #Combine A and B picks (keeping this in case want to add another call later)
        all_picks=b_picks
        all_epochs=b_epochs
        all_freq=b_freq
        all_type=btype
        ...
        [snr,ambient_snr] = detect.get_snr(all_picks, t, f_sub, Sxx_sub, utcstart_chunk,snr_limits=[35, 40],snr_calllength=1,snr_freqwidth=3,dur=2)
        ...
        frames = [df for df in [Brydes_calls, Brydes_calls_sta]
                  if not df.empty and not df.isna().all().all()]
        Brydes_calls = pd.concat(frames)
        #from ChatGPT to fix Future Warning of Concantenation

       #Brydes_calls = Brydes_calls.append(Brydes_calls_sta)
        #import pdb; pdb.set_trace()
        Brydes_calls.to_csv(CHUNK_FILE, index=False) #writes .csv chunk file
...
#Writes final .csv file      
    ...
    final_analyzer_df = Brydes_calls
